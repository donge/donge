...
conn.execute("update [bbsconfig] set lasttopic='<a onclick=min_yuzi() target=message href=ShowPost.asp?id="&id&">"&left(topic,10)&"</a>',lastname='"&username&"',lasttime='"&now()&"',today=today+1,tolrestore=tolrestore+1 where id="&forumid&"")
...
<td align=right><%=banner%></td>
	...
	<table border="0" width="97%" align="center" cellspacing="1" cellpadding="4" class=a2>
		<tr class=a3>
			<td height="25">&nbsp;<img src=images/Forum_nav.gif>&nbsp; <a href=main.asp><%=clubname%></a> → <%ForumTree(followid)%><%=ForumTreeList%> <a href=ShowForum.asp?forumid=<%=forumid%>><%=bbsname%></a> → <a onclick=min_yuzi() target=message href="ShowPost.asp?id=<%=id%>"><%=Request("topic")%></a> → 回复帖子</td>
		</tr>
	</table><br>
