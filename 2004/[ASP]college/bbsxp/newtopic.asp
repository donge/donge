...
if Application(CacheName&"content")=content then
...
conn.execute("update [bbsconfig] set lasttopic='<a onclick=min_yuzi() target=message href=ShowPost.asp?id="&id&">"&left(topic,10)&"</a>',lastname='"&username&"',lasttime='"&now()&"',today=today+1,toltopic=toltopic+1,tolrestore=tolrestore+1 where id="&forumid&"")
conn.execute("update [clubconfig] set today=today+1")


Application(CacheName&"content") = content
...
ActivationForum="<a onclick=min_yuzi() target=message href=ShowPost.asp?id="&id&">返回主题</a>"
end if

message="<li>新主题发表成功<li>"&ActivationForum&"<li><a href=ShowForum.asp?forumid="&forumid&">返回论坛</a><li><a href=main.asp>返回论坛首页</a>"
...
<br>
	<table border="0" width="97%" align="center" cellspacing="1" cellpadding="4" class=a2>
		<tr class=a3>
			<td height="25">&nbsp;<img src=images/Forum_nav.gif>&nbsp; <a href=main.asp><%=clubname%></a> → <%ForumTree(followid)%><%=ForumTreeList%> <a href=ShowForum.asp?forumid=<%=forumid%>><%=bbsname%></a> → 发表帖子</td>
		</tr>
	</table><br>
